...
def run():
    """Runs gtf_parser.py and creates transcript objects for the input GTF file.
    ...
    parser.add_argument('gi', metavar='gtf_input', help = "The GTF annotation input file name")
...
class transcript(object):
    """An object for a given transcript id in the GTF file.  Will be in the output."""

    def __init__(self, transcript_id, seqname, strand, frame, gene_id_attributes, gene_id, source = None):
    ...
    def add_exon(self, exon):
        """Adds an exon to the transcript object.  Throws an exception if the exon is out of order in the GTF file, or when it overlaps another exon.  If an exception is thrown for a given transcript, it will not be included for the output."""

        if exon[0] > exon[1]:
            raise Exception("Invalid exon start/stop in transcript: " + str(self.transcript_id))

        if self.furthestAddedExon < exon[0]: # Add an exon to the end (i.e. exons are in order)
            self.exons += [exon]
        ...
        for e in self.exons:
            if (e[1] >= self.exons[0]) and (self.exons[1] >= e[0]):
                raise Exception("Overlapping exon in transcript: " + str(self.transcript_id))
        index = bisect_left(self.exons, exon) # Add an exon elsewhere, to keep exons sorted
    ...
    def compatible(self, pos):
        """Returns the exon index if 'pos' is inside one of the exons. Else, returns -1"""
    ...
    def __str__(self):
        return '{0}\t{1}\t{2}\t{3}'.format(self.transcript_id, self.refname, 
                self.front_coordinate, self.end_coordinate)

def gtf_parse(input_gtf):
    """Parses the input GTF file by line.  Creates a list of transcript objects that include each transcript's id, refname, strand, frame, gene id, and its exons.  Uses 1-based inclusive coordinates.  For example, start: 1, end: 2 would be an exon of length two (at 1, 2)."""
    ...
    for line in gtf_file:
        ...
        try:
            transcript_id = (gtf_line[8].split(";")[1].split(" ")[2].replace("\"",""))
        ...
        if current_transcript is None:
            try:
                ...
                gene_id = gtf_line[8].split(";")[0].split(" ")[1].replace("\"","")
                current_transcript = transcript(transcript_id, gtf_line[0], gtf_line[6], gtf_line[7], gtf_line[8].split(" ")[3], gene_id, gtf_line[1])
                transcript_dictionary[transcript_id] = current_transcript
                transcript_dictionary[transcript_id].add_exon((int(gtf_line[3]), int(gtf_line[4])))
            except IndexError as i:
            ...
            try:
                ...
                gene_id = gtf_line[8].split(";")[0].split(" ")[1].replace("\"","")
                # FIXME: gene_id_attributes isn't being parsed correctly
                current_transcript = transcript(transcript_id, gtf_line[0], gtf_line[6], gtf_line[7], gtf_line[8].split(" ")[4:], gene_id, gtf_line[1])
                transcript_dictionary[transcript_id] = current_transcript
                transcript_dictionary[transcript_id].add_exon((int(gtf_line[3]), int(gtf_line[4])))
            ...
            try:
                current_transcript.add_exon((int(gtf_line[3]), int(gtf_line[4])))
            except IndexError as i:
                print "GTF File Input missing fields"
            except Exception as e:
                print(e)
    ...
    print >> sys.stderr, ("Number of transcript objects made: " + str(len(transcript_dictionary.keys())))
